...
const burger = {
	...
	indirim: function (indirimUygulanacakMüsteriTipi) {
		return indirimUygulanacakMüsteriTipi === "öğretmen" ||
			indirimUygulanacakMüsteriTipi === "öğrenci"
			? this.fiyat * 0.75
			: this.fiyat * 0.9;
	},
};
...
degerlendirmeler.forEach((deger) =>
	deger.isim === "Ahmet" ? deger.geribildirim : false
);
...
function AnahtardanDegerlendirmeAl(sampleArray, sampleIndex) {
	return `${sampleArray[sampleIndex].isim} isimli kişi ${sampleArray[sampleIndex].puan} puan verdi ve şunları yazdı: ${sampleArray[sampleIndex].geribildirim}`;
}
...
function SonDegerlendirmeyiAl(degerlendirmelerDizisi) {
	...
	return (
		...
		"isimli kişi " +
		sonMusterPuanı +
		"puanverdi ve şunları yazdı: " +
		sonMusteriGeriBildirimi
	);
}
...
function PuanaGoreDegerlendirmeAl(degerlendirmeler, samplePuan) {
	let sampleRange = Math.floor(samplePuan);
	degerlendirmeler.filter((deger) => {
		deger.puan >= samplePuan && deger.puan < sampleRange;
		console.log(deger);
	});
}
...
function UzunDegerlendirmeleriAl(sampleArray) {
	const newArray = [];

	newArray.push(
		sampleArray.forEach((sample) => {
			sample.geribildirim.length > 15 ? sample.geribildirim : false;
		})
	);
